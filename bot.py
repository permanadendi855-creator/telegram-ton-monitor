...
def menu_markup() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("💰 Info Saldo", callback_data="balance")],
            [InlineKeyboardButton("🟣 10 Transaksi TON", callback_data="tx_ton")],
            [InlineKeyboardButton("🪙 10 Transaksi USDT", callback_data="tx_usdt")],
            [InlineKeyboardButton("🪙 Token Dimiliki", callback_data="tokens")],
...
async def get_account_state() -> dict[str, Any]:
    ...
    if accounts:
        return accounts[0]
    account_states = data.get("account_states", [])
    if account_states:
...
def token_info_from_response(response: dict[str, Any], jetton_address: str) -> dict[str, Any]:
    ...
    if not isinstance(metadata, dict):
        return {}
    for key in (jetton_address, jetton_address.replace("-", "_")):
...
def token_decimals(info: dict[str, Any], default: int = 9) -> int:
...
def normalize_ton_events(tx: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    source = friendly_address(in_msg.get("source"))
    value = int(in_msg.get("value") or 0)
    if (source and source!= "-" and source!= friendly_address(WALLET_ADDRESS) and value > 0 and not aborted):
        events.append({"kind": "ton", "direction": "in", "symbol": "TON", "amount": format_ton(value), "source": source, "destination": (friendly_address(WALLET_ADDRESS)), "counterparty": source, "timestamp": now, "lt": lt, "hash": tx_hash, "event_id": (f"ton-in:{tx_hash}"), "aborted": aborted})
    for index, msg in enumerate(tx.get("out_msgs") or []):
        destination = friendly_address(msg.get("destination"))
        msg_value = int(msg.get("value") or 0)
        if (destination and destination!= "-" and destination!= friendly_address(WALLET_ADDRESS) and msg_value > 0 and not aborted):
...
async def get_recent_usdt_events(limit: int = 10) -> list[dict[str, Any]]:
    ...
    for item in incoming:
        event = normalize_usdt_event(item, "in")
        if not event["aborted"]:
            events.append(event)
    for item in outgoing:
        event = normalize_usdt_event(item, "out")
        if not event["aborted"]:
            events.append(event)
    return sort_and_unique_events(events, limit)
...
async def show_ton_transactions(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        lines = ["🟣 <b>10 TRANSAKSI TON TERAKHIR</b>", ""]
        if not events:
            lines.append("Tidak ada transaksi TON.")
        else:
            for index, event in enumerate(events, 1):
                lines.append(format_history_event(event, index))
                lines.append("")
        await query.edit_message_text("\n".join(lines), parse_mode=ParseMode.HTML, reply_markup=back_markup(), disable_web_page_preview=True)
...
async def show_usdt_transactions(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    await query.answer("Mengambil 10 transaksi USDT...")
    try:
        events = (await get_recent_usdt_events(10))
        lines = ["🪙 <b>10 TRANSAKSI USDT TERAKHIR</b>", ""]
        if not events:
            lines.append("Tidak ada transaksi USDT.")
        else:
            for index, event in enumerate(events, 1):
                lines.append(format_history_event(event, index))
                lines.append("")
...
async def send_event_notification(application: Application, event: dict[str, Any]) -> None:
    ...
    timestamp = fmt_time(event.get("timestamp"))
    tx_hash = (event.get("hash") or "")
    text = ("🚨 <b>TRANSAKSI BARU</b>\n\n" f"{icon} <b>{label}</b>\n\n" f"💰 Jumlah: <b>{sign}{html_escape(amount)} {html_escape(symbol)}</b>\n\n" f"📤 Pengirim:\n<code>{html_escape(source)}</code>\n\n" f"📥 Penerima:\n<code>{html_escape(destination)}</code>\n\n" f"📅 {html_escape(timestamp)} {html_escape(TIMEZONE_NAME)}\n")
    if tx_hash:
        text += (f'\n🔗 <a href="{explorer_url(tx_hash)}">Lihat transaksi di Tonviewer</a>')
    failed: list[str] = []
    for chat_id in list(monitor_chats):
        try:
            ...
            logger.warning("Gagal mengirim notifikasi ke %s: %s", chat_id, exc)
...
async def show_balance(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    await query.answer("Mengambil saldo...")
    try:
        state, wallets_response = (await asyncio.gather(get_account_state(), get_jetton_wallets(100)))
        ton_raw = (state.get("balance") or "0")
        ton_balance = format_ton(ton_raw)
        wallets = (wallets_response.get("jetton_wallets", []))
        lines = ["💰 <b>INFO SALDO</b>", "", f"TON: <b>{html_escape(ton_balance)} TON</b>", "", "🪙 <b>JETTON</b>"]
        if not wallets:
            lines.append("Tidak ada Jetton dengan saldo &gt; 0.")
        else:
            for wallet in wallets[:30]:
                master = (wallet.get("jetton") or "")
                info = (token_info_from_response(wallets_response, master))
                symbol = (info.get("symbol") or ("USDT" if master == USDT_JETTON_MASTER else "JETTON"))
                name = (info.get("name") or symbol)
                decimals = (6 if master == USDT_JETTON_MASTER else token_decimals(info, 9))
                balance = (format_amount(wallet.get("balance", "0"), decimals))
                lines.append(f"• <b>{html_escape(symbol)}</b> — {html_escape(balance)}\n {html_escape(name)}")
        lines.extend(["", "Wallet:\n<code>{html_escape(WALLET_ADDRESS)}</code>", f"🕐 Update: {html_escape(fmt_time(time.time()))} {html_escape(TIMEZONE_NAME)}"])
        await query.edit_message_text("\n".join(lines), parse_mode=ParseMode.HTML, reply_markup=back_markup(), disable_web_page_preview=True)
    except Exception as exc:
...
async def show_tokens(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        response = (await get_jetton_wallets(100))
        wallets = (response.get("jetton_wallets", []))
        lines = ["🪙 <b>TOKEN YANG DIMILIKI</b>", ""]
        if not wallets:
            lines.append("Tidak ada Jetton dengan saldo &gt; 0.")
        else:
            for i, wallet in enumerate(wallets[:50], 1):
                master = (wallet.get("jetton") or "")
                info = (token_info_from_response(response, master))
                symbol = (info.get("symbol") or ("USDT" if master == USDT_JETTON_MASTER else "JETTON"))
                name = (info.get("name") or symbol)
                decimals = (6 if master == USDT_JETTON_MASTER else token_decimals(info, 9))
                balance = (format_amount(wallet.get("balance", "0"), decimals))
                lines.append(f"<b>{i}. {html_escape(symbol)}</b> — {html_escape(balance)}\n {html_escape(name)}\n Master: <code>{html_escape(master)}</code>")
        await query.edit_message_text("\n".join(lines), parse_mode=ParseMode.HTML, reply_markup=back_markup(), disable_web_page_preview=True)
    except Exception as exc:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "balance":
        ...
        await show_ton_transactions(update, context)
    elif data == "tx_usdt":
        await show_usdt_transactions(update, context)
    elif data == "tokens":
...
async def post_init(application: Application) -> None:
    ...
    await application.bot.set_my_commands([BotCommand("start", "Buka menu utama"), BotCommand("menu", "Buka menu utama"), BotCommand("chatid", "Lihat Chat ID")])
